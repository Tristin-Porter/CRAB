...
public class TemplateExpander
{
    ...
    private void InitializeTemplates()
    {
        switch (architecture)
        {
            ...
            case "x86_16":
                templates["push"] = ctx => Badger.Architectures.x86_16.WATToX86_16MapSet.ExpandPush();
                templates["pop"] = ctx => Badger.Architectures.x86_16.WATToX86_16MapSet.ExpandPop(ctx.GetValueOrDefault("dest", "ax"));
                templates["pop2"] = ctx => Badger.Architectures.x86_16.WATToX86_16MapSet.ExpandPop2();
                break;
...
public class BadgerCompiler
{
    ...
    public static byte[] Compile(string watInput, string architecture = "x86_64", string format = "native")
    {
        try
        {
            ...
            // For native/PE formats, generate actual working binary from WAT
            byte[] machineCode = GenerateNativeMachineCode(watInput, architecture);
            ...
            byte[] binary = format.ToLower() switch
            {
                ...
                "elf" => Badger.Containers.ELF.Emit(machineCode),
    ...
    /// <summary>
    /// Generate native assembly from WAT input
    /// Parses WAT and creates assembly with console I/O support
    /// </summary>
    private static string GenerateNativeAssembly(string watInput, string architecture)
    {
        // For now, directly generate machine code instead of assembly
        // This is simpler than implementing a full assembler
        return ""; // Placeholder - we'll generate machine code directly
    }
    
    /// <summary>
    /// Generate native machine code from WAT input
    /// Parses WAT and creates executable machine code with console I/O
    /// </summary>
    private static byte[] GenerateNativeMachineCode(string watInput, string architecture)
    {
        // Parse WAT to extract data section and console_log calls
        var dataStrings = new List<(int offset, string data)>();
        var consoleLogCalls = new List<(int ptr, int len)>();
        
        // Simple regex-based parsing of data sections
        var dataMatches = System.Text.RegularExpressions.Regex.Matches(
            watInput, 
            @"\(data\s+\(i32\.const\s+(\d+)\)\s+""([^""\\]*(?:\\.[^""\\]*)*)""\)"
        );
        
        foreach (System.Text.RegularExpressions.Match match in dataMatches)
        {
            int offset = int.Parse(match.Groups[1].Value);
            string data = match.Groups[2].Value;
            // Unescape the string
            data = data.Replace("\\00", "\0").Replace("\\n", "\n").Replace("\\r", "\r")
                      .Replace("\\t", "\t").Replace("\\\"", "\"").Replace("\\\\", "\\");
            dataStrings.Add((offset, data));
        }
        
        // Find console_log calls in Main function
        var logCallMatches = System.Text.RegularExpressions.Regex.Matches(
            watInput,
            @"i32\.const\s+(\d+)\s+i32\.const\s+(\d+)\s+call\s+\$console_log"
        );
        
        foreach (System.Text.RegularExpressions.Match match in logCallMatches)
        {
            int ptr = int.Parse(match.Groups[1].Value);
            int len = int.Parse(match.Groups[2].Value);
            consoleLogCalls.Add((ptr, len));
        }
        
        // Generate architecture-specific machine code
        return architecture.ToLower() switch
        {
            "x86_64" => GenerateX86_64MachineCode(dataStrings, consoleLogCalls),
            _ => throw new ArgumentException($"Architecture {architecture} not yet implemented for native backend")
        };
    }
    
    /// <summary>
    /// Generate x86-64 Linux machine code with console I/O
    /// Directly generates binary instructions without assembly step
    /// </summary>
    private static byte[] GenerateX86_64MachineCode(List<(int offset, string data)> dataStrings, List<(int ptr, int len)> consoleLogCalls)
    {
        var code = new List<byte>();
        var strings = new Dictionary<int, byte[]>();
        
        // Convert strings to bytes
        foreach (var (offset, data) in dataStrings)
        {
            strings[offset] = System.Text.Encoding.UTF8.GetBytes(data);
        }
        
        // For each console_log call, generate:
        // 1. Jump over embedded string
        // 2. Embedded string data
        // 3. sys_write syscall
        
        foreach (var (ptr, len) in consoleLogCalls)
        {
            if (!strings.TryGetValue(ptr, out var strBytes))
                continue;
            
            // Get the actual string bytes (up to len)
            var actualBytes = strBytes.Take(len).ToArray();
            
            int stringStartPos = code.Count + 2; // After JMP instruction
            
            // JMP short forward (skip string): EB <offset>
            // We need to jump len bytes forward
            if (actualBytes.Length < 128)
            {
                code.Add(0xEB); // JMP short
                code.Add((byte)actualBytes.Length);
            }
            
            // Embed string bytes
            code.AddRange(actualBytes);
            
            int codeStartPos = code.Count;
            
            // sys_write syscall
            // mov rax, 1 (sys_write)
            code.AddRange(new byte[] { 0x48, 0xC7, 0xC0, 0x01, 0x00, 0x00, 0x00 }); // 7 bytes
            
            // mov rdi, 1 (stdout)
            code.AddRange(new byte[] { 0x48, 0xC7, 0xC7, 0x01, 0x00, 0x00, 0x00 }); // 7 bytes
            
            // lea rsi, [rip + offset]  (point back to the string)
            // RIP after this instruction will be at position (codeStartPos + 14 + 7)
            // We want to point to stringStartPos
            // So offset = stringStartPos - (codeStartPos + 14 + 7)
            int leaPos = code.Count;
            int ripAfterLea = leaPos + 7; // LEA instruction is 7 bytes
            int ripOffset = stringStartPos - ripAfterLea;
            
            code.AddRange(new byte[] { 0x48, 0x8D, 0x35 }); // lea rsi, [rip + offset]
            code.AddRange(BitConverter.GetBytes(ripOffset));
            
            // mov rdx, len (length)
            code.AddRange(new byte[] { 0x48, 0xC7, 0xC2 });
            code.AddRange(BitConverter.GetBytes(len).Take(4).ToArray());
            
            // syscall
            code.AddRange(new byte[] { 0x0F, 0x05 });
        }
        
        // Exit syscall: mov rax, 60; xor rdi, rdi; syscall
        code.AddRange(new byte[] { 0x48, 0xC7, 0xC0, 0x3C, 0x00, 0x00, 0x00 }); // mov rax, 60
        code.AddRange(new byte[] { 0x48, 0x31, 0xFF }); // xor rdi, rdi
        code.AddRange(new byte[] { 0x0F, 0x05 }); // syscall
