...
public class WASM : MapSet
{
    ...
    // ============================================================
    // INSTANCE HELPER METHODS FOR USING CDTk's Transform API
    // ============================================================

    /// <summary>
    /// Transform an AST node using CDTk's Map system.
    /// This is the proper way to process AST nodes - it delegates to the appropriate Map
    /// based on the node's type, allowing CDTk to handle the recursive transformation.
    /// </summary>
    private string TransformNode(object? node)
    {
        if (node == null) return "";
        
        // Handle List<AstNode> - transform each and join
        if (node is List<AstNode> astList)
        {
            return string.Join("\n", astList.Select(n => this.Transform(n) ?? ""));
        }
        
        // Handle List<object> - transform each and join
        if (node is List<object> objList)
        {
            return string.Join("\n", objList.Select(o => {
                if (o is AstNode astNode)
    ...
    /// <summary>
    /// Generate the WASM data section from registered string literals.
    /// This uses the StringRegistry to emit all string constants.
    /// </summary>
    private string GenerateDataSection()
    {
        var sb = new System.Text.StringBuilder();
        var allStrings = WASM.StringRegistry.GetAllStrings();
        
        foreach (var kvp in allStrings.OrderBy(x => x.Key))
        {
            var stringId = kvp.Key;
            var text = kvp.Value;
            var offset = WASM.StringRegistry.GetStringOffset(stringId);
    ...
    /// <summary>
    /// Escape special characters in string literals for WASM output.
    /// </summary>
    ...
    /// <summary>
    /// Emit a string literal node by registering it and returning WASM code.
    /// This is called from the StringLiteral Map.
    /// </summary>
    private string EmitStringLiteral(AstNode node)
    {
        var text = node.Fields.ContainsKey("lexeme") ? node.Fields["lexeme"]?.ToString() ?? "" : "";
        ...
        var stringId = WASM.StringRegistry.RegisterString(text);
        var offset = WASM.StringRegistry.GetStringOffset(stringId);
    ...
    public Map<AstNode, string> CompilationUnit = TypedMap.For<string>()
        .Emit(node => {
            ...
            var dataSection = this.GenerateDataSection();
    ...
    private static string EmitMethodDeclarationInline(AstNode node)
    {
        ...
        if (bodyField is AstNode bodyNode)
        {
            ...
            {
                body = this.TransformNode(bodyNode.Fields["body"]);
            }
            else
            {
                body = this.TransformNode(bodyNode);
            }
        }
...
{elements}
    ...
    public Map<AstNode, string> StringLiteral = TypedMap.For<string>()
        .Emit(node => this.EmitStringLiteral(node));
